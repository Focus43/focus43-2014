<?php defined('C5_EXECUTE') or die("Access Denied.");
    ...
    new InstagramAPI(function( InstagramAPI $instance ) use ($itemsPerPage){
        $response = $instance->getRecentMedia($itemsPerPage);
        ...
        foreach($response->data AS $mediaObj){ ?>
            <div class="gram" style="background-image:url('<?php echo $mediaObj->images->standard_resolution->url; ?>');">
                <div class="tabular">
                    <div class="cellular">
                        <span class="caption"><?php echo !empty($mediaObj->caption->text) ? $mediaObj->caption->text : 'No caption - imagine something clever...'; ?></span>
                    </div>
                </div>
